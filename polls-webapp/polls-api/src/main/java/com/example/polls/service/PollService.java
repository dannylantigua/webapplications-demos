...
public class PollService {
    ...
    public PagedResponse<PollResponse> getAllPolls(UserPrincipal currentUser, int page, int size) {
        ...
        if (polls.getNumberOfElements() == 0){
            return new PagedResponse<>(Collections.emptyList(), polls.getNumber(),
                    polls.getSize(), polls.getTotalElements(), polls.getTotalPages(), polls.isEmpty());
        }
        ...
        List<PollResponse> pollResponses = polls.map(poll -> {
            return ModelMapper.mapPollToPollRespone(poll,
                    choiceVoteCountMap,
    ...
    public PagedResponse<PollResponse> getPollsVotedBy(String username, UserPrincipal currentUser, int page, int size){
        ...
        // Map Polls to PollResponses containing vote counts and poll creator details
        Map<Long, Long> choiceVotedCountMap = getChoiceVoteCountMap(pollIds);
        Map<Long, Long> pollUserVoteMap = getPollUserVoteMap(currentUser, pollIds);
    ...
    public PollResponse getPollById(Long pollId, UserPrincipal currentUser) {
        ...
        return ModelMapper.mapPollToPollRespone(poll, choiceVotesMap,
    ...
    public  PollResponse castVoteAndGetUpdatedPoll(Long pollId, VoteRequest voteRequest, UserPrincipal currentUser){
        ...
        return ModelMapper.mapPollToPollRespone(poll, choiceVotesMap, creator, vote.getChoice().getId());
    }
    ...
    Map<Long, User> getPollCreatorMap(List<Poll> polls) {
        ...
        List<Long> creatorsId = polls.stream()
                .map(Poll::getCreatedBy)
                .distinct()
                .collect(Collectors.toList());

        List<User> creators = userRepository.findByIdIn(creatorsId);
